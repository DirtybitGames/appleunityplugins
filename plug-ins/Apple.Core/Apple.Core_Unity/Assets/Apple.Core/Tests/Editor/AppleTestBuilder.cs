...
namespace Apple.Core.Tests.Editor
{
    public class AppleTestBuilder : ITestPlayerBuildModifier, IPostBuildCleanup
    {
        ...
        {
            Debug.Log("com.dirtybit.apple.unityplugin.core: Modifying player options in the test build");
        ...
        {
            Debug.Log("com.dirtybit.apple.unityplugin.core: Entered cleanup. Checking if commandline run...");
            if (s_RunningPlayerTests && IsRunningTestsFromCommandline())
            {
                Debug.Log("com.dirtybit.apple.unityplugin.core: Exiting Editor.");
                EditorApplication.update += () => { EditorApplication.Exit(0); };
            }
            Debug.Log("com.dirtybit.apple.unityplugin.core: Exiting Cleanup.");
        }
